...
% Last Modified by GUIDE v2.5 19-Sep-2019 13:03:17
...
function choose_tab(hObject, handles, idx)
    ...
    for i = 1:tab_num
        button= handles.(['tab', num2str(i)]);
        group = handles.(['uibuttongroup', num2str(i)]);
        P = get(handles.uipanel1, 'Position');
        set(group, 'Position', [0 0 P(3) P(4)]);
        group.HighlightColor = [0.94 0.94 0.94];
        group.ShadowColor = [0.94 0.94 0.94];
        if (i==idx)  % 
           % disp(button);
           % disp(group);
            button.BackgroundColor = [1 1 1];                 % 高亮按钮
            group.Visible = 'on';                                              % 显示group
        else 
            button.BackgroundColor = [0.94 0.94 0.94];  % 关闭button
            group.Visible = 'off';                                               % 隐藏group  
        end 
    end 
